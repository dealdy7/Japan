        ...
        ═══════════════════════════════════════════ */
        const PLAN_OPTIONS = [
            { id: 'A', label: 'Budget Traveler', icon: '🎒', totalIDR: 8000000, flight: 3600000, hotel: 1700000, food: 1200000, transport: 900000, souvenir: 600000, color: '#3ddc84', colorDim: 'rgba(61,220,132,.1)', colorBorder: 'rgba(61,220,132,.22)', desc: 'Perjalanan hemat maksimal — konbini, hostel, dan IC Card.', style: 'Hostel / Capsule Hotel', stars: '★★' },
            { id: 'B', label: 'Mid Traveler', icon: '🧳', totalIDR: 14000000, flight: 5500000, hotel: 4500000, food: 2000000, transport: 1200000, souvenir: 800000, color: '#f4a7b9', colorDim: 'rgba(244,167,185,.1)', colorBorder: 'rgba(244,167,185,.22)', desc: 'Keseimbangan kenyamanan & budget — hotel bintang 3, restoran.', style: 'Business Hotel ★★★', stars: '★★★' },
            { id: 'C', label: 'Comfortable', icon: '✈️', totalIDR: 22000000, flight: 8000000, hotel: 9000000, food: 3000000, transport: 1500000, souvenir: 500000, color: '#d4af37', colorDim: 'rgba(212,175,55,.1)', colorBorder: 'rgba(212,175,55,.22)', desc: 'Pengalaman premium — hotel bintang 4-5 dan fine dining.', style: 'Hotel ★★★★-★★★★★', stars: '★★★★★' },
        ];
  ...
  `).join('');
        ...
        function setPrepTab(tab) {
            state.prepTab = tab;
            document.getElementById('btn-before').classList.toggle('active', tab === 'before');
            document.getElementById('btn-after').classList.toggle('active', tab === 'after');
            renderPrep();
        }
        ...
        function renderTips() { renderAccordions(TIPS_ITEMS, 'tips-accordions'); }
        ...
        function renderPlanOptions() {
            document.getElementById('plan-options').innerHTML = PLAN_OPTIONS.map(opt => {
      ...
      <div class="plan-option-card${active ? ' active' : ''}" onclick="selectPlan('${opt.id}')"
        ...
        <div style="font-size:12px;color:var(--t2);line-height:1.6;margin-bottom:1.1rem">${opt.desc}</div>
        <div style="display:flex;flex-direction:column;gap:5px">
          ${[{ l: 'Total Budget', v: fmtIDR(opt.totalIDR), bold: true }, { l: '✈️ Tiket Pesawat', v: fmtIDR(opt.flight) }, { l: '🏨 Hotel (7 malam)', v: fmtIDR(opt.hotel) }, { l: '🍱 Makan', v: fmtIDR(opt.food) }, { l: '🚆 Transport', v: fmtIDR(opt.transport) }, { l: '🎁 Souvenir', v: fmtIDR(opt.souvenir) }].map(({ l, v, bold }) => `
            <div class="plan-option-row${bold ? ' plan-option-total' : ''}">
              <span class="plan-option-label">${l}</span>
              <span class="plan-option-val" style="${bold ? 'color:' + opt.color + ';font-size:15px;font-weight:700' : 'color:var(--t2)'}">${v}</span>
            </div>
          `).join('')}
        </div>
        <div style="margin-top:1rem;text-align:center;padding:8px;border-radius:9px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);font-size:11px;color:var(--t3)">
          ${currencyMode === 'dual' ? `≈ ${jpy(Math.round(opt.totalIDR / IDR))} total JPY` : ''}
        </div>
      </div>
    `;
            }).join('');
        ...
        function renderPlanBanner() {
            ...
            document.getElementById('plan-banner').style.cssText = `background:linear-gradient(135deg,${opt.colorDim},rgba(255,255,255,.02));border:1px solid ${opt.colorBorder};border-radius:16px;padding:1.4rem 1.75rem;margin-bottom:2rem;display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap`;
            document.getElementById('plan-banner').innerHTML = `
    <div style="font-size:2.5rem">${opt.icon}</div>
    <div style="flex:1;min-width:200px">
      <div style="font-family:'Playfair Display';font-size:1.3rem;font-weight:700;color:var(--t)">${opt.label} — Option ${opt.id}</div>
      <div style="font-size:13px;color:var(--t2);margin-top:3px">${opt.desc}</div>
    </div>
    <div style="text-align:right">
      <div style="font-size:11px;color:var(--t3)">Total Budget</div>
      <div style="font-family:'Playfair Display';font-size:1.9rem;font-weight:700;color:${opt.color}">${fmtIDR(opt.totalIDR)}</div>
      <div style="font-size:11px;color:var(--t3)">7 hari • ${opt.style}</div>
    </div>
  `;
        }
